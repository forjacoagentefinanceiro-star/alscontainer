...
function parseXML(xml: string): { items: ParsedContainer[], errors: string[] } {
  ...
  containerEls.forEach((el, idx) => {
    try {
      ...
      items.push({ numero, tipo, nacionalizado: false, tamanho, fornecedor, data_compra, ano_fabricacao: null, valor_usd, cotacao, extras_brl, valor_brl, obs, iso_valido: isoValido })
    } catch (e) {
