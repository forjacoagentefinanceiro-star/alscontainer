      ...
      <div className="bg-white rounded-lg overflow-hidden" style={{ border: '1px solid #e5e7eb', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
        {filtered.length === 0 ? (
          <div className="text-center py-16" style={{ color: '#9ca3af' }}>
            ...
            <p className="text-sm">{containers.length === 0 ? 'Nenhum container cadastrado.' : 'Nenhum resultado para os filtros.'}</p>
            {containers.length === 0 && (
            ...
            <tbody>
              {filtered.map((c, i) => (
                <tr key={c.id} style={{ borderBottom: '1px solid #f3f4f6', background: i % 2 === 0 ? '#fff' : '#fafafa' }}
                  ...
                  <td className="px-4 py-3 text-xs" style={{ color: '#6b7280', maxWidth: 120 }}>
