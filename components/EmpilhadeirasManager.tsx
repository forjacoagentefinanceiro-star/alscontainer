...
export function EmpilhadeirasManager({ empilhadeiras, defaultOpen = false }: { empilhadeiras: Empilhadeira[]; defaultOpen?: boolean }) {
  ...
  function add() {
    ...
    startTransition(async () => {
      const res = await addEmpilhadeira(n, setor)
      if (!res.error) {
        setList(prev => [...prev, {
  ...
  function salvarEdicao(id: string) {
    ...
    if (!n) return
    startTransition(async () => {
      const res = await updateEmpilhadeira(id, n, editSetor)
      if (!res.error) {
        setList(prev => prev.map(e => e.id === id ? { ...e, nome: n, setor: editSetor.trim() || null } : e)
          .sort((a, b) => a.nome.localeCompare(b.nome)))
        setEditId(null)
      }
    })
      ...
      {open && (
        <div className="p-5">
          ...
          <div className="flex gap-2 mb-4 flex-wrap">
            ...
            <input value={setor} onChange={e => setSetor(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') add() }}
              className={`w-36 ${inp}`} style={{ borderColor: '#d1d5db', color: '#1a2a3a' }}
                  ...
                  {editId === e.id ? (
                    <>
                      <input value={editNome} onChange={ev => setEditNome(ev.target.value)}
                        ...
                        autoFocus className="flex-1 min-w-28 rounded border px-2 py-1.5 text-sm outline-none" style={{ borderColor: '#1B4F8A', color: '#1a2a3a' }} />
                      <input value={editSetor} onChange={ev => setEditSetor(ev.target.value)}
                        onKeyDown={ev => { if (ev.key === 'Enter') salvarEdicao(e.id); if (ev.key === 'Escape') setEditId(null) }}
                        className="w-32 rounded border px-2 py-1.5 text-sm outline-none" style={{ borderColor: '#d1d5db', color: '#1a2a3a' }}
                        placeholder="Setor" />
                    ...
                    <>
                      <span className="flex-1 text-sm" style={{ color: '#1a2a3a' }}>
                        {e.nome}
                        {e.setor && (
                          <span className="ml-2 text-xs font-medium px-1.5 py-0.5 rounded" style={{ background: '#eff6ff', color: '#1d4ed8', border: '1px solid #bfdbfe' }}>
                            {e.setor}
                          </span>
                        )}
